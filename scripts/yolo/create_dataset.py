...
class DipoleRotator:
    """
    Priority 1: Physics-Aware Domain Randomization.
    Re-simulates dipole signatures for physically valid orientations.
    """
    def __init__(self, inclination=60.0, declination=0.0):
        self.inc = inclination
        self.dec = declination

    def generate_valid_anomaly(self, pos, moment, rel_points):
        """Analytically re-computes TMI anomaly without invalid image rotations."""
        # pos: (e, n, z), moment: (mx, my, mz)
        bx, by, bz = hm.dipole_magnetic(rel_points, pos, moment, field="b")
        tmi = hm.total_field_anomaly((bx, by, bz), inclination=self.inc, declination=self.dec)
        return tmi
...
def shift_cibles_to_anomaly_peak(gdf_cibles, src_raster):
    ...
    # Observer points relative to center
    rel_obs_points = np.array([rel_xx.ravel(), rel_yy.ravel(), np.zeros_like(rel_xx.ravel())])
    
    rotator = DipoleRotator(inclination=FIELD_INCLINATION, declination=FIELD_DECLINATION)
    shifted_points = []
    ...
    for index, row in gdf_cibles.iterrows():
        ...
        mx, my, mz = np.random.normal(0, 1, 3)
        ...
        tmi_anomaly = rotator.generate_valid_anomaly(dipole_pos, DIPOLE_MOMENT, (obs_e, obs_n, obs_z))
            ...
            with tarfile.open(archive_path, "w:gz") as tar:
                for r in range(0, height, STRIDE):
                    for c in range(0, width, STRIDE):
                        ...
                        # v62: YOLO Bounding Box generation (Priority 1)
                        # Format: class x_center y_center width height (normalized)
                        yolo_labels = []
                        if not disks_in_window.empty:
                            for geom in disks_in_window.geometry:
                                # geom is a disk centered on the peak
                                # Get peak in local patch coordinates
                                peak_lat, peak_lon = geom.centroid.y, geom.centroid.x
                                # Transform back to pixel coords in full image
                                px_c, px_r = ~src.transform * (peak_lon, peak_lat)
                                
                                # Local pixel coords in patch
                                rel_px_c = px_c - c
                                rel_px_r = px_r - r
                                
                                # Normalized [0, 1]
                                x_center = rel_px_c / PATCH_SIZE
                                y_center = rel_px_r / PATCH_SIZE
                                
                                # Fixed box size: 2.0m -> 20 pixels at 0.1m res
                                w_norm = 20.0 / PATCH_SIZE
                                h_norm = 20.0 / PATCH_SIZE
                        ...
                        label_io = io.BytesIO()
                        label_content = "\n".join(yolo_labels).encode('utf-8')
                        label_io.write(label_content)
                        label_io.seek(0)
                        tar_info = tarfile.TarInfo(name=f"labels/{patch_name}_image.txt")
                        tar_info.size = len(label_content)
                        tar.addfile(tar_info, label_io)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-prefix", default="processed_v62_yolo_physics")
