...
def predict_on_raster(input_raster_path, output_raster_path, model_path, batch_size=16):
    ...
    if abs(res_x - TARGET_RES) > 1e-3 or abs(res_y - TARGET_RES) > 1e-3:
        local_input = resample_raster(local_input, "/tmp/resampled_input_v60.tif", target_res=TARGET_RES)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f"Using device: {device}")
    
    # v60: UNet (CNN Stability Refinement)
    ...
    local_output = "/tmp/output_raster_v60.tif"
    ...
    with rasterio.open(local_input) as src:
        ...
        rows = range(0, src.height, STRIDE)
        for r_offset in tqdm(rows, desc="Rows"):
            patches = []
            windows = []
            for c_offset in range(0, src.width, STRIDE):
                ...
                valid_pixels = patch[~nodata_mask]
                mean_val = np.mean(valid_pixels) if valid_pixels.size > 0 else 0
                ...
                patch_tensor = TF.normalize(patch_tensor, [0], [300]) # Roughly normalize to [0, 1] range based on clip
                
                # Native resolution for UNet in v60 is 256x256, no resize needed.
            ...
            for i in range(0, len(patches), batch_size):
                ...
                # UNet returns (logits4, logits3, logits2)
                logits, _, _ = model(batch_tensors)
                probs = torch.sigmoid(logits)
                
                probs = probs.cpu().numpy().squeeze(1) # (B, H, W)
    ...
    if output_raster_path.startswith("gs://"):
        ...
        shutil.move(local_output, output_raster_path)
    
    logging.info(f"Prediction complete. Heatmap saved to {output_raster_path}")
