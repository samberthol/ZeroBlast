...
from model import SegFormerWrapper
...
def predict_on_raster(input_raster_path, output_raster_path, model_path, batch_size=16):
    ...
    # v62: SegFormerWrapper
    model = SegFormerWrapper(n_channels=1, n_classes=1)
    ...
    local_output = f"/tmp/output_segformer_{os.path.basename(input_raster_path)}"
    ...
    with rasterio.open(local_input) as src:
        ...
        for r_offset in tqdm(range(0, src.height, STRIDE)):
            ...
            for c_offset in range(0, src.width, STRIDE):
                ...
                patch_tensor = torch.from_numpy(patch_hp).unsqueeze(0).unsqueeze(0) # (1, 1, H, W)
                patch_tensor = TF.normalize(patch_tensor, [0], [300])
                
                patches.append(patch_tensor.squeeze(0))
            ...
            for i in range(0, len(patches), batch_size):
                ...
                logits = model.predict(batch_tensors)
